...
from backend.coords import global_tile_to_face, tile_pixel_box, face_pixel_to_lonlat, get_projection_type
...
app.add_middleware(
    ...
    allow_headers=["*"],
    expose_headers=[
        "X-Width", "X-Height", "X-Components", "X-Dtype",
        "X-Face", "X-Projection",
        "X-Face-Width", "X-Face-Height",
        "X-Face-X0", "X-Face-X1", "X-Face-Y0", "X-Face-Y1",
        "X-Lon00", "X-Lat00", "X-Lon10", "X-Lat10",
        "X-Lon01", "X-Lat01", "X-Lon11", "X-Lat11"
    ],
)
...
# Global buffer manager (created on startup, started on first play)
...
async def _startup_buffer():
    ...
    # Create data manager with disk caching
    try:
        manager = MultiFaceDataManager()
        logger.info("MultiFaceDataManager initialized - tiles will be loaded on-demand with disk caching")
    except Exception:
        logger.exception("Failed to initialize MultiFaceDataManager on startup")
        return
    
    # Create BufferManager (activated on first play)
    ...
    logger.info("Server ready - all tiles loaded on-demand")
...
def get_tile(
    ...
    try:
        ...
        # Debug log to verify face calculation
        logging.getLogger("backend.server").info(f"Tile z={z_i} x={x} y={y} -> face={face_id}, local=({local_tx},{local_ty}), tiles_per_face={tiles_per_face}")
        ...
        # Compute ACTUAL dimensions from returned data (which may be downsampled by max_pixels)
        byte_len = len(data_bytes)
        float_count = byte_len // 4
        components = 2
        pixels = float_count // components
        
        # Find factors of pixel count - try common square-ish dimensions first
        import math
        width = int(math.sqrt(pixels))
        
        # Find nearest factor
        while width > 0 and pixels % width != 0:
            width -= 1
        
        height = pixels // max(1, width) if width > 0 else pixels
        
        # Compute lon/lat bounds for this tile using gnomonic projection
        # Map tile pixel corners to face pixel positions for accurate projection
        # Tile pixel (i, j) maps to face pixel: lx0 + (i+0.5)/(width) * (lx1-lx0)
        
        # For corner pixels of the returned tile:
        # Top-left (0, 0) -> face pixel at lx0 + 0.5 * dx/width, ly0 + 0.5 * dy/height
        # Top-right (width-1, 0) -> face pixel at lx0 + (width-0.5) * dx/width, ly0 + 0.5 * dy/height
        # etc.
        
        dx = lx1 - lx0  # face pixel width of this tile region
        dy = ly1 - ly0  # face pixel height of this tile region
        
        # Corners at pixel centers
        face_px00 = lx0 + 0.5 * dx / width
        face_py00 = ly0 + 0.5 * dy / height
        
        face_px10 = lx0 + (width - 0.5) * dx / width
        face_py10 = ly0 + 0.5 * dy / height
        
        face_px01 = lx0 + 0.5 * dx / width
        face_py01 = ly0 + (height - 0.5) * dy / height
        
        face_px11 = lx0 + (width - 0.5) * dx / width
        face_py11 = ly0 + (height - 0.5) * dy / height
        
        try:
            lon00, lat00 = face_pixel_to_lonlat(face_id, face_px00, face_py00, face_W, face_H)
            lon10, lat10 = face_pixel_to_lonlat(face_id, face_px10, face_py10, face_W, face_H)
            lon01, lat01 = face_pixel_to_lonlat(face_id, face_px01, face_py01, face_W, face_H)
            lon11, lat11 = face_pixel_to_lonlat(face_id, face_px11, face_py11, face_W, face_H)
            
            logging.getLogger("backend.server").info(
                f"Tile coords: face={face_id}, corners=({lon00:.2f},{lat00:.2f})->({lon11:.2f},{lat11:.2f})"
            )
        except Exception as e:
            logging.getLogger("backend.server").error(f"Error computing coords: {e}")
            lon00 = lon10 = lon01 = lon11 = 0.0
            lat00 = lat10 = lat01 = lat11 = 0.0
        
        proj_type = get_projection_type(face_id)
        ...
        headers = {
            ...
            "X-Dtype": "float32",
            "X-Face": str(face_id),
            "X-Projection": proj_type,
            # Face dimensions for coordinate computation
            "X-Face-Width": str(face_W),
            "X-Face-Height": str(face_H),
            # Tile position in face pixel coordinates
            "X-Face-X0": str(lx0),
            "X-Face-X1": str(lx1),
            "X-Face-Y0": str(ly0),
            "X-Face-Y1": str(ly1),
